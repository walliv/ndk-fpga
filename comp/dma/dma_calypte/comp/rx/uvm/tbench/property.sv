...
module DMA_LL_PROPERTY  #(DEVICE, USR_MFB_REGIONS, USR_MFB_REGION_SIZE, USR_MFB_BLOCK_SIZE, USR_MFB_ITEM_WIDTH, PCIE_RQ_REGIONS, PCIE_RQ_REGION_SIZE, PCIE_RQ_BLOCK_SIZE, PCIE_RQ_ITEM_WIDTH, CHANNELS, PKT_SIZE_MAX)
    ...
    property ptr_upd_sof_eof_src_rdy;
        @(posedge ptr_upd_mfb.CLK) disable iff(RESET)
        (ptr_upd_mfb.SRC_RDY && (ptr_upd_mfb.SOF != 0)) |-> ptr_upd_mfb.SRC_RDY s_until_with (ptr_upd_mfb.EOF != 0);
    endproperty

    assert property (ptr_upd_sof_eof_src_rdy)
        else begin
            `uvm_error(module_name, "\n\tPointer Update interface. MFB To PCIE must'n stop sending data in middle of frame");
        end
endmodule
